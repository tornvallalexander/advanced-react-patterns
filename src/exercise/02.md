...
Implicitly adding state to children of a component. The main idea of this is
to map over the children and add the state in the parent component.

Just based off of this idea, what would make sense to do is something like this:
...
However, this will result in an error, since the object isn't extensible.
Kind of makes sense that we should not be able to just change props like that.

What we then have to do instead is something like this:
...
function Toggle({children}) {
  ...
  return React.Children.map(children, child => {
    // using the cloneElement method works splendid here when 
...
Although this works great for the original case, problem arises when we add other
elements that are not meant to have that state, or don't need it.

For instance, when we add a normal `span` element, it will result in an error.
The way we get around this is by checking the type of the element, or selectively
adding the props to certain children.
