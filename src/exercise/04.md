...
What is very interesting about prop getters is the ability to consume state
values from a hook as in context, but being able to do it without any wrapping
component. Is this how it should work? What are the limitations?
...
Seems like there is slight difference between prop getters and contexts. With
prop getters we can add props to child components, but with contexts we can
consume the values directly in that child component, meaning we don't need
to pass the values as props.
