...
When we have to make multiple changes to state and keep track of those changes,
the pattern below can be very useful. Instead of manipulating state directly
in a `handleSubmit` function, we extract that logic into a separate utility function.
...
This comes with a lot of advantages explained below. But most notably
is the ability to still being able to easily do side effects by awaiting
the value and returning a function.

Below is how we would later use our implemented function along with handling
errors.
